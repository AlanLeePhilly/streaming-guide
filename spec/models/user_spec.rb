...
RSpec.describe User, type: :model do
  ...
  it 'has a matching password confirmation for the password' do
    user = User.new_user_session_pathuser.password = 'password'
    user.password_confirmration = 'anotherpasword'

    expect(user).to_not be_valid
    expect(user.errors[:password_confirmration]).to_not be_blank
  end
